...
import org.role.rPG.UI.Ui; // 필요하다면 이 리스너도 등록
...
public class LIS_manager {
     ...
     */
    public void registerGeneralListeners(StatManager statManager, IndicatorManager indicatorManager, ItemManager itemManager, LevelManager levelManager, MobManager mobManager) {

        // GUI 이벤트 총괄 매니저
        pm.registerEvents(new GUI_manager(), plugin);

        // 기타 모든 리스너들
        pm.registerEvents(new Stat(plugin, statManager, indicatorManager), plugin);
        pm.registerEvents(new Cooked(plugin), plugin);
        pm.registerEvents(new EquipmentListener(plugin, statManager), plugin);
        // pm.registerEvents(new Ui(plugin, statManager, levelManager), plugin); // Ui 클래스가 리스너라면 등록
